    ...
    Effect.gen(function* () {
      ...
      const bytesRequest = HttpClientRequest.post(`${baseUrl}/upload-bytes`).pipe(
        HttpClientRequest.bodyUint8Array(
          encoder.encode("upload-bytes"),
          "application/octet-stream",
        ),
      );
      const bytesResponse = yield* client
        .execute(bytesRequest)
        .pipe(Effect.flatMap(HttpClientResponse.schemaBodyJson(UploadBytesResponse)));
      if (
        bytesResponse.length !== 12 ||
        !bytesResponse.contentType.includes("application/octet-stream")
      ) {
      ...
      const streamRequest = HttpClientRequest.post(`${baseUrl}/upload-bytes`).pipe(
        HttpClientRequest.bodyStream(
          Stream.make(encoder.encode("upload-"), encoder.encode("stream")),
          {
      ...
      const streamResponse = yield* client
        .execute(streamRequest)
        .pipe(Effect.flatMap(HttpClientResponse.schemaBodyJson(UploadBytesResponse)));
      if (
        streamResponse.length !== 13 ||
        !streamResponse.contentType.includes("application/octet-stream")
      ) {
      ...
      const formResponse = yield* client
        .execute(formRequest)
        .pipe(Effect.flatMap(HttpClientResponse.schemaBodyJson(UploadFormResponse)));
      if (
        formResponse.length <= 0 ||
        !formResponse.contentType.toLowerCase().includes("multipart/form-data")
      ) {
        throw new Error(`upload form contract failed: ${JSON.stringify(formResponse)}`);
      }
      if (
        !formResponse.bodyPreview.includes("field") ||
        !formResponse.bodyPreview.includes("value")
      ) {
      ...
      if (typeof FormData !== "undefined" && typeof File !== "undefined") {
        ...
        data.append(
          "file",
          new File([encoder.encode("blob-data")], "file.txt", { type: "text/plain" }),
        );
        const formFileRequest = HttpClientRequest.post(`${baseUrl}/upload-form`).pipe(
          HttpClientRequest.bodyFormData(data),
        );
        const formFileResponse = yield* client
          .execute(formFileRequest)
          .pipe(Effect.flatMap(HttpClientResponse.schemaBodyJson(UploadFormResponse)));
        ...
        if (
          ...
          throw new Error(
            `upload file preview contract failed: ${JSON.stringify(formFileResponse)}`,
          );
        }
      }
...
export const verifyAdvancedUploadHttpClientLayer = async (
  ...
  await Effect.runPromise(
    Effect.gen(function* () {
      ...
      const streamRequest = HttpClientRequest.post(`${baseUrl}/upload-bytes`).pipe(
        HttpClientRequest.bodyStream(Stream.fromIterable(streamParts)),
      );
      const streamResponse = yield* client
        .execute(streamRequest)
        .pipe(Effect.flatMap(HttpClientResponse.schemaBodyJson(UploadBytesResponse)));
      if (
        streamResponse.length !== streamLength ||
        !streamResponse.contentType.includes("application/octet-stream")
      ) {
        throw new Error(
          `advanced stream upload contract failed: ${JSON.stringify(streamResponse)}`,
        );
      }
      ...
      const multiFieldResponse = yield* client
        .execute(multiFieldRequest)
        .pipe(Effect.flatMap(HttpClientResponse.schemaBodyJson(UploadFormResponse)));
      if (!multiFieldResponse.contentType.toLowerCase().includes("multipart/form-data")) {
        throw new Error(
          `advanced multipart content type failed: ${JSON.stringify(multiFieldResponse)}`,
        );
      }
      if (
        ...
        throw new Error(
          `advanced multipart field preview failed: ${JSON.stringify(multiFieldResponse)}`,
        );
      }
      ...
      if (typeof FormData !== "undefined" && typeof File !== "undefined") {
        ...
        data.append(
          "fileA",
          new File([encoder.encode("file-a")], "file-a.txt", { type: "text/plain" }),
        );
        data.append(
          "fileB",
          new File([encoder.encode("file-b")], "file-b.txt", { type: "text/plain" }),
        );
        const multiFileRequest = HttpClientRequest.post(`${baseUrl}/upload-form`).pipe(
          HttpClientRequest.bodyFormData(data),
        );
        const multiFileResponse = yield* client
          .execute(multiFileRequest)
          .pipe(Effect.flatMap(HttpClientResponse.schemaBodyJson(UploadFormResponse)));
        if (!multiFileResponse.contentType.toLowerCase().includes("multipart/form-data")) {
          throw new Error(
            `advanced multipart file content type failed: ${JSON.stringify(multiFileResponse)}`,
          );
        }
        if (
          ...
          throw new Error(
            `advanced multipart file preview failed: ${JSON.stringify(multiFileResponse)}`,
          );
        }
      }
      ...
      const largeResponse = yield* client
        .execute(largeRequest)
        .pipe(Effect.flatMap(HttpClientResponse.schemaBodyJson(UploadBytesResponse)));
  ...
  await Effect.runPromise(
    Effect.gen(function* () {
      ...
      const malformed = HttpClientRequest.post(`${baseUrl}/upload-form-strict`).pipe(
        HttpClientRequest.bodyUint8Array(
          encoder.encode("missing-boundary"),
