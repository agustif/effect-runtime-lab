  ...
  await Effect.runPromise(
    Effect.gen(function* () {
      ...
      if (actual.length !== expected.length) {
        throw new Error(`watch sequence contract failed: expected ${expected.length} events, got ${actual.length}`)
      }
      for (let index = 0; index < expected.length; index++) {
        const left = actual[index]!
        const right = expected[index]!
        if (left._tag !== right._tag || left.path !== right.path) {
          throw new Error(`watch sequence contract failed at ${index}: ${JSON.stringify(left)} !== ${JSON.stringify(right)}`)
        }
      }
  ...
  await Effect.runPromise(
    Effect.gen(function* () {
      ...
      if (finalIndex < expected.length) {
        ...
        throw new Error(
          `watch subsequence contract failed: expected ${expected.length} events, got ${finalIndex}. Observed: ${
...
export const verifyWatchRenameLayer = async (
  ...
  await Effect.runPromise(
    Effect.gen(function* () {
      ...
      if (Option.isNone(timed)) {
        const events = yield* Ref.get(observed)
        throw new Error(`watch rename timed out after ${options.timeout ?? "2 seconds"}: ${JSON.stringify(events)}`)
      }
      if (pending.size > 0) {
        const events = yield* Ref.get(observed)
        throw new Error(
