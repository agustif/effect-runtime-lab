...
export const verifyBasicRuntimeMain = async (
  makeHarness: () => RuntimeMainHarness
) => {
  {
    ...
    if (harness.exitCodes.length !== 0) {
      throw new Error(`runtime main success must not force exit(0): ${JSON.stringify(harness.exitCodes)}`)
    }
    ...
    if (!harness.registrations.some((entry) => entry.op === "remove" && entry.signal === "SIGINT")) {
      throw new Error("runtime main must remove SIGINT")
    }
    if (!harness.registrations.some((entry) => entry.op === "remove" && entry.signal === "SIGTERM")) {
...

  {
    ...
    if (harness.exitCodes[0] !== 7) {
      throw new Error(`runtime main failure exit contract failed: ${JSON.stringify(harness.exitCodes)}`)
    }
  }

  {
    ...
    if (harness.exitCodes[0] !== 130) {
      throw new Error(`runtime main interrupt exit contract failed: ${JSON.stringify(harness.exitCodes)}`)
    }
  }
}
