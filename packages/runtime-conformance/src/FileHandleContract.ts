...
const resolveSystemTag = (error: PlatformError.PlatformError): PlatformError.SystemErrorTag | "BadArgument" =>
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const initial = yield* file.readAlloc(FileSystem.Size(11)).pipe(Effect.flatMap((_) => _.asEffect()))
          ...
          if (decode(preservedCursorResult) !== "lorem Xpsum") {
            throw new Error(`file-handle truncate cursor preservation failed: ${decode(preservedCursorResult)}`)
          }
          ...
          if (decode(clampedCursorResult) !== "lorem ipsum!") {
            throw new Error(`file-handle truncate cursor clamp failed: ${decode(clampedCursorResult)}`)
          }
        })
      )
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const firstRead = yield* file.readAlloc(FileSystem.Size(3)).pipe(Effect.flatMap((_) => _.asEffect()))
          ...
          const secondRead = yield* file.readAlloc(FileSystem.Size(6)).pipe(Effect.flatMap((_) => _.asEffect()))
          if (decode(secondRead) !== "barbaz") {
            throw new Error(`file-handle append cursor persistence contract failed: ${decode(secondRead)}`)
          }
        })
      )
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const middle = yield* file.readAlloc(FileSystem.Size(2)).pipe(Effect.flatMap((_) => _.asEffect()))
          if (decode(middle) !== "cd") {
