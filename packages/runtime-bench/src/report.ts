...
import { fileSizeBytes, measureCommand, platformDependencies, withTemporaryJavaScriptFile, type CommandMeasurement } from "./index.js"
...
> = [
  { name: "quickjs-host", mode: "script", command: resolve(repoRoot, "packages/platform-quickjs/bin/effect-quickjs-host") },
  { name: "txiki-runtime", mode: "script", command: resolve(repoRoot, ".tmp-runtime-evals/txikijs/build/tjs"), args: ["run"] },
  { name: "txiki-compiled", mode: "standalone", command: resolve(repoRoot, ".tmp-runtime-evals/txikijs/hello-compiled") },
...
const measureCandidate = (
  candidate: (typeof startupCandidates)[number]
): CommandMeasurement => {
...
export const collectWarmStart = (iterations = 5): Array<WarmStartRecord> => {
  ...
  for (const candidate of startupCandidates) {
    ...
    try {
      ...
      const rssSource = measurements.find((measurement) => measurement.rssSource !== "unknown")?.rssSource ?? "unknown"
      out.push({
        name: candidate.name,
        iterations,
        samplesMs,
...
export const writeRuntimeBenchSnapshot = (outputPath = resolve(repoRoot, "docs/generated/runtime-bench.json")) => {
