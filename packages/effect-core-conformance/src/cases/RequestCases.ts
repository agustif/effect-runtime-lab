...
const makeUserResolver = Effect.gen(function* () {
  ...
  const resolver = Resolver.make<GetAllIds | GetNameById>(Effect.fnUntraced(function*(entries) {
    counter.count++
...
export const requestsDontBreakInterruptionCase = () =>
  Effect.gen(function*() {
    ...
    if (counter.count !== 0 || interrupts.interrupts !== 1) {
      throw new Error(`request interruption state mismatch: ${JSON.stringify({ counter, interrupts })}`)
    }
  }).pipe(
    provideEnv,
...
export const preservesIdenticalRequestsCase = () =>
  Effect.gen(function* () {
    ...
    if (requests.count !== 2 || invocations.count !== 1) {
      throw new Error(`request duplicate preservation failed: ${JSON.stringify({ requests, invocations })}`)
    }
  }).pipe(provideEnv)
...
export const groupedResolverKeepsKeysSeparatedCase = () =>
  Effect.gen(function* () {
    ...
    const resolver = Resolver.make<GroupedRequest>(Effect.fnUntraced(function*(entries) {
      const group = entries[0].request.group
      if (!entries.every((entry) => entry.request.group === group)) {
        throw new Error(`grouped resolver mixed keys: ${JSON.stringify(entries.map((entry) => entry.request.group))}`)
      }
    ...
    if (JSON.stringify(results) !== JSON.stringify(expected)) {
      throw new Error(`grouped resolver results mismatch: ${JSON.stringify({ expected, results })}`)
    }
    ...
    for (const { group, count } of [
      ...
      if (total !== count) {
        throw new Error(`grouped resolver count mismatch for ${group}: ${JSON.stringify({ total, count })}`)
      }
    }
  })
