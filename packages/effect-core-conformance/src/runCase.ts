...
const classifyFailure = (error: string): FailureType => {
  ...
  if (message.includes("not implemented") || message.includes("not available") || message.includes("unsupported")) {
...
export const runCase = async (name: string, run: () => Promise<void>): Promise<CuratedCaseResult> => {
