...
import { parallelFinalizationCase, sequentialFinalizersRunInReverseOrderCase } from "./cases/ScopeCases.js"
import {
...
export const curatedCases: ReadonlyArray<CuratedCaseDefinition> = [
  ...
  { name: "scope.sequentialFinalizersRunInReverseOrder", run: () => runCuratedEffect(sequentialFinalizersRunInReverseOrderCase()) },
  { name: "request.batchingPreservesRequests", run: () => runCuratedEffect(batchingPreservesRequestsCase()) },
  { name: "request.groupedResolverKeepsKeysSeparated", run: () => runCuratedEffect(groupedResolverKeepsKeysSeparatedCase()) },
  { name: "request.preservesIdenticalRequests", run: () => runCuratedEffect(preservesIdenticalRequestsCase()) },
  { name: "request.requestsDontBreakInterruption", run: () => runCuratedEffect(requestsDontBreakInterruptionCase()) },
  ...
  { name: "fiberset.joinCompletesOnFailureWithRunningFibers", run: () => runCuratedEffect(joinCompletesOnFailureWithRunningFibersCase()) },
  { name: "fiberset.runtime", run: () => runCuratedEffect(runtimeCase()) },
  { name: "fiberset.size", run: () => runCuratedEffect(sizeCase()) },
  { name: "fiberset.sizeDropsAfterCompletion", run: () => runCuratedEffect(sizeDropsAfterCompletionCase()) },
  { name: "fiberset.propagateInterruptionFalse", run: () => runCuratedEffect(propagateInterruptionFalseCase()) },
  { name: "fiberset.propagateInterruptionTrue", run: () => runCuratedEffect(propagateInterruptionTrueCase()) },
  ...
  { name: "queue.interruptAllowsDraining", run: () => runCuratedEffect(interruptAllowsDrainingCase()) },
  ...
  { name: "queue.shutdownCompletesAwait", run: () => runCuratedEffect(shutdownCompletesAwaitCase()) },
  ...
  { name: "stream.callbackHandlesErrors", run: () => runCuratedEffect(callbackHandlesErrorsCase()) },
  { name: "stream.callbackHandlesDefects", run: () => runCuratedEffect(callbackHandlesDefectsCase()) },
  { name: "stream.aggregateWithinInterruptPropagation", run: () => runCuratedEffect(aggregateWithinInterruptPropagationCase()) },
  { name: "stream.aggregateWithinDefectPropagation", run: () => runCuratedEffect(aggregateWithinDefectPropagationCase()) }
]
...
export const collectInventory = () => {
  ...
  return {
    ...
    adopted: upstreamCases.filter((entry) => entry.category === "adopted").map((entry) => ({
      source: basename(entry.source),
      reason: entry.reason
    })),
    next: upstreamCases.filter((entry) => entry.category === "next").map((entry) => ({
      source: basename(entry.source),
      reason: entry.reason
    })),
    exploratory: upstreamCases.filter((entry) => entry.category === "exploratory").map((entry) => ({
      source: basename(entry.source),
      reason: entry.reason
    }))
  }
