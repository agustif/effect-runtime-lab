...
export const upstreamCases: ReadonlyArray<UpstreamCase> = [
  ...
  },
  {
    ...
    category: "adopted",
    reason: "temp, copy, truncate, cursor, append, and basic file-handle semantics are ported into shared filesystem contracts"
  },
  {
  ...
  },
  {
    ...
    category: "adopted",
    reason: "interruption, request-service propagation, batching, duplicate preservation, and grouped-resolver semantics are covered by the curated runner"
  },
  {
    source: "vendor/effect-smol/packages/effect/test/FiberSet.test.ts",
    category: "adopted",
    reason: "interrupt, awaitEmpty, join, runtime, size, and propagateInterruption semantics are covered by the curated runner"
  },
  {
    source: "vendor/effect-smol/packages/effect/test/Queue.test.ts",
    category: "adopted",
    reason: "interrupt, offerAll interruption, done, end, poll, and shutdown semantics are covered by the curated runner"
  },
  {
    source: "vendor/effect-smol/packages/effect/test/Stream.test.ts",
    category: "adopted",
    reason: "callback cleanup, backpressure, end, error, defect, and aggregateWithin propagation semantics are covered by the curated runner"
  },
  {
    source: "vendor/effect-smol/packages/effect/test/Scope.test.ts",
    category: "adopted",
    reason: "parallel finalization and reverse-order sequential finalizer semantics are covered by the curated runner"
  },
  {
    source: "vendor/effect-smol/packages/platform-node/test/NodeHttpClient.test.ts",
    category: "next",
    reason: "still the main source for richer local HTTP parity such as schema decoding, HEAD, and broader streaming semantics"
  },
  {
    source: "vendor/effect-smol/packages/effect/test/Effect.test.ts",
    category: "exploratory",
    reason: "larger runtime-sensitive effect semantics still need a curated import strategy beyond the current focused runner"
  }
]
