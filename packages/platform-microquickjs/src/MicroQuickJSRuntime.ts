...
export const evalEffect = (runtime: MicroQuickJSRuntime, code: string, options?: EvalOptions) =>
  Effect.tryPromise({
    try: () => runtime.eval(code, options),
    catch: (cause) => runtimeError("eval", "guest evaluation failed", cause),
  });

export const exposeEffect = (runtime: MicroQuickJSRuntime, name: string, object: HostObject) =>
  Effect.tryPromise({
...
export const layer = (options?: MicroQuickJSCreateOptions) =>
  Layer.effect(MicroQuickJSEngine)(
    Effect.acquireRelease(createEffect(options), (runtime) => Effect.sync(() => runtime.dispose())),
  );
