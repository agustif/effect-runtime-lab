...
const multipartEntriesOf = (value: unknown): Array<MultipartEntry> => {
  ...
  if (typeof value.entries === "function") {
    return Array.from(value.entries()).map(
      ([name, entryValue]) => [name, entryValue, undefined] as const,
    );
  }
  return [];
};
...
describe("QuickJSHttpClient", () => {
  ...
  it("satisfies the shared redirect contract with a mocked host", async () => {
    await ConformanceRedirect.verifyRedirectHttpClientLayer(
  ...
  it("passes QuickJS-specific options through to the host request", async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const response = yield* configured
          .pipe(HttpClient.followRedirects())
          .get("http://fixture/redirect");
  ...
  it("satisfies the shared upload contract with a mocked host", async () => {
    await ConformanceUpload.verifyAdvancedUploadHttpClientLayer(
      QuickJSHttpClient.layer,
      "http://fixture",
    );
  });
