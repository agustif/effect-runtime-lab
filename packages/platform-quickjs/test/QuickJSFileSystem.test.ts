...
beforeEach(() => {
  ...
  (globalThis as typeof globalThis & { os?: unknown }).os = {
    ...
    open(path: string, flags = 0) {
      ...
      handles.set(fd, {
        path,
        position: (flags & 1024) !== 0 ? (files.get(path)?.byteLength ?? 0) : 0,
      });
      return fd;
    },
    close(fd: number) {
...
describe("QuickJSFileSystem", () => {
  ...
  it("emits ordered create/update/remove events through the shared watch contract", async () => {
    ...
    await ConformanceWatch.verifyWatchSubsequenceLayer(QuickJSFileSystem.layer, dir, [
  ...
  it("emits rename events through the shared watch contract", async () => {
    ...
    await ConformanceWatch.verifyWatchRenameLayer(QuickJSFileSystem.layer, dir, original, renamed);
  });
  ...
  it("emits watch events for nested directories", async () => {
    ...
    await ConformanceWatch.verifyWatchChurnLayer(QuickJSFileSystem.layer, nested, [
      { _tag: "Create", path: file },
      { _tag: "Remove", path: file },
    ]);
  });
