...
describe("QuickJS deferred parity surfaces", () => {
  it("implements watch semantics beyond the old deferred policy", () => {
    ...
    expect(tasks).not.toContain("- [ ] richer watch semantics beyond the current create/update/remove and polling-based contract")
  })
  ...
  it("covers multipart form-data semantics", () => {
    const tasks = readFileSync(resolve(packageRoot, "TASKS.md"), "utf8")
    expect(tasks).not.toContain("- [ ] multipart depth beyond the current basic form upload contract")
  })
  ...
  it("documents cross-platform artifact guarantees with executable proofs", () => {
    ...
    expect(bench.artifacts.some((entry) => entry.path.includes("platform-quickjs/bin/effect-quickjs-host"))).toBe(true)
