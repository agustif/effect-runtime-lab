...
const main = Effect.gen(function* () {
  ...
  const tempDir = yield* fs.makeTempDirectory({ prefix: "prod-qjs-" });
  const tempFile = yield* fs.makeTempFile({ prefix: "prod-qjs-", suffix: ".txt" });
  ...
  yield* fs.remove(`${dir}/renamed.txt`);
