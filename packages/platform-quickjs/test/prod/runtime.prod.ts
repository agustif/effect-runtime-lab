...
const main = Effect.gen(function* () {
  ...
  const args = yield* Stdio.Stdio.pipe(Effect.flatMap((stdio) => stdio.args), Effect.provide(QuickJSStdio.layer))
  ...
  const timer = yield* Effect.promise(() => new Promise<string>((resolve) => setTimeout(() => resolve("ok"), 5)))
  ...
  yield* Stream.run(Stream.make("stdout smoke"), Stdio.Stdio.pipe(Effect.map((stdio) => stdio.stdout()), Effect.provide(QuickJSStdio.layer)))
