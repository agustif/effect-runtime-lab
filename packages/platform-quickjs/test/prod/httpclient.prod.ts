...
const main = Effect.gen(function* () {
  ...
  if (
    typeof redirectedJson !== "object" ||
    redirectedJson === null ||
    !("message" in redirectedJson)
  ) {
  ...
  const uploaded = yield* HttpClientRequest.post(`${baseUrl}/upload-bytes`).pipe(
    HttpClientRequest.bodyUint8Array(
      new TextEncoder().encode("upload-bytes"),
  ...
  const uploadedStream = yield* HttpClientRequest.post(`${baseUrl}/upload-bytes`).pipe(
    HttpClientRequest.bodyStream(
      Stream.make(new TextEncoder().encode("upload-"), new TextEncoder().encode("stream")),
      {
  ...
  fileData.append(
    "file",
    new File([new TextEncoder().encode("blob-data")], "file.txt", { type: "text/plain" }),
  );
  ...
  multiFileData.append(
    "fileA",
    new File([new TextEncoder().encode("file-a")], "file-a.txt", { type: "text/plain" }),
  );
  multiFileData.append(
    "fileB",
    new File([new TextEncoder().encode("file-b")], "file-b.txt", { type: "text/plain" }),
  );
  ...
  yield* Effect.promise(() =>
    ConformanceHttp.verifyHttpErrorMappingLayer(QuickJSHttpClient.layer, baseUrl, {
      configureTimeout: (client) =>
        client.pipe(QuickJSHttpClient.withOptions({ timeoutSeconds: 0.05 })),
    }),
  );
