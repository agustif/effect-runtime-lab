...
export const withOptions: {
  ...
  HttpClient.transformResponse(self, Effect.updateService(QuickJSOptions, (current) => ({ ...current, ...options })))
)
...
class QuickJSResponse extends Inspectable.Class implements HttpClientResponse.HttpClientResponse, Pipeable {
  ...
  constructor(
    ...
    this.initialBody = source.bodyBytes
      ...
      : new Uint8Array(0)
    this.readChunk = typeof source.readChunk === "function" ? source.readChunk.bind(source) : undefined
  }
  ...
  get cookies(): Cookies.Cookies {
    ...
    return this._cookies = setCookie ? Cookies.fromSetCookie(Array.isArray(setCookie) ? setCookie : [setCookie]) : Cookies.empty
  }
  ...
  get text(): Effect.Effect<string, HttpClientError.HttpClientError> {
    return this.textBody ??= Effect.tryPromise({
      ...
      catch: (cause) => decodeError(this.request, this, "response body text decoding failed", cause)
  ...
  get urlParamsBody(): Effect.Effect<UrlParams.UrlParams, HttpClientError.HttpClientError> {
    return Effect.flatMap(this.text, (text) =>
      Effect.try({
        ...
        catch: (cause) => decodeError(this.request, this, "response body is not valid URLSearchParams", cause)
      })
    )
  }
  ...
  get arrayBuffer(): Effect.Effect<ArrayBuffer, HttpClientError.HttpClientError> {
    return this.arrayBufferBody ??= Effect.tryPromise({
      ...
      catch: (cause) => decodeError(this.request, this, "response body arrayBuffer decoding failed", cause)
    }).pipe(Effect.cached, Effect.runSync)
  }
...
): Effect.Effect<string | Uint8Array | ArrayBuffer | null | FormData, HttpClientError.HttpClientError> => {
  switch (body._tag) {
    case "Empty":
    ...
    case "Stream":
      return Stream.runCollect(body.stream).pipe(
        ...
        Effect.mapError((cause) => transportError(request, "stream request body encoding failed", cause))
      )
  }
}
...
const makeClient = HttpClient.make((request, url, signal, fiber) =>
  Effect.gen(function* () {
    ...
    if (!http) {
      return yield* Effect.fail(transportError(request, "QuickJS HTTP host module is not available"))
    }
    ...
    const response = yield* Effect.tryPromise({
      try: () =>
        http.request(request.method, url.toString(), encodedBody, headerString, options.timeoutSeconds ?? 30, {
          signal,
          stream: true,
