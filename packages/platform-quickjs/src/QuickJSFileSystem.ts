...
declare const globalThis: {
  ...
  readonly setInterval?: (handler: (...args: Array<unknown>) => void, timeout?: number, ...args: Array<unknown>) => unknown
...
const getOs = (method: string): Effect.Effect<QuickJSOsModule, PlatformError.PlatformError> =>
  globalThis.os ? Effect.succeed(globalThis.os) : Effect.fail(moduleError(method, "QuickJS os module not available"))
...
const diffWatchSnapshots = (
  ...
  if (previous._tag === "File" || current._tag === "File") {
    ...
    if (previous._tag === "File" && current._tag === "File" && !watchEntryEquals(previous.entry, current.entry)) {
  ...
  if (previous._tag === "Missing" && current._tag === "Directory") {
    return Array.from(current.entries.keys(), (child) => ({ _tag: "Create", path: joinPath(rootPath, child) }) as const)
  }
  if (previous._tag === "Directory" && current._tag === "Missing") {
    return Array.from(previous.entries.keys(), (child) => ({ _tag: "Remove", path: joinPath(rootPath, child) }) as const)
  }
...
const readDirectoryRecursive = (os: QuickJSOsModule, path: string): Effect.Effect<Array<string>, PlatformError.PlatformError> =>
  Effect.gen(function* () {
...
const removeRecursive = (os: QuickJSOsModule, path: string): Effect.Effect<void, PlatformError.PlatformError> =>
  Effect.gen(function* () {
    const [info, errno] = os.stat(path)
    if (errno !== 0) {
  ...
  Effect.gen(function* () {
    ...
    if (options?.recursive) {
      return yield* Effect.matchEffect(
        removeRecursive(os, path),
        {
          onFailure: (error) =>
            options?.force && error.reason._tag === "NotFound"
              ? Effect.void
              : Effect.fail(error),
  ...
  Effect.gen(function* () {
    ...
    try {
      ...
      while (offset < data.byteLength) {
        const written = os.write(fd, getBackingBuffer(data), data.byteOffset + offset, data.byteLength - offset)
        if (written < 0) {
...
const copyRecursive = (os: QuickJSOsModule, fromPath: string, toPath: string): Effect.Effect<void, PlatformError.PlatformError> =>
  Effect.gen(function* () {
...
const makeTempDirectory: FileSystem.FileSystem["makeTempDirectory"] = (options) =>
  Effect.gen(function* () {
    ...
    if (!os.mkdtemp) {
      return yield* Effect.fail(moduleError("makeTempDirectory", "mkdtemp is not available in the host"))
    }
    const [path, errno] = os.mkdtemp(joinPath(tempBase(options?.directory), tempTemplate(options?.prefix)))
...
const makeTempFile: FileSystem.FileSystem["makeTempFile"] = (options) =>
  Effect.gen(function* () {
    ...
    if (!os.mkstemp) {
      return yield* Effect.fail(moduleError("makeTempFile", "mkstemp is not available in the host"))
    }
    const [path, errno] = os.mkstemp(joinPath(tempBase(options?.directory), tempTemplate(options?.prefix, options?.suffix ?? "")))
    if (errno !== 0 || path === null) {
      ...
      const file: FileSystem.File = {
        ...
        read(buffer) {
          return Effect.gen(function* () {
            ...
            const bytesRead = os.read(fd, getBackingBuffer(buffer), buffer.byteOffset, buffer.byteLength)
            if (bytesRead < 0) {
        ...
        write(buffer) {
          return Effect.gen(function* () {
            const targetOffset = append ? os.seek(fd, 0, seekEnd(os)) : os.seek(fd, cursor, seekStart(os))
            if (targetOffset < 0) {
              return yield* Effect.fail(systemError("write", path)(targetOffset))
            }
            let localOffset = 0
            while (localOffset < buffer.byteLength) {
              const written = os.write(fd, getBackingBuffer(buffer), buffer.byteOffset + localOffset, buffer.byteLength - localOffset)
              if (written < 0) {
  ...
  Stream.scoped(
    Stream.fromPull(
      Effect.gen(function* () {
        ...
        let remaining = options?.bytesToRead === undefined ? undefined : Math.max(0, Number(FileSystem.Size(options.bytesToRead)))
