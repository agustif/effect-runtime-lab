...
export const withOptions: {
  (options: { readonly timeoutMs?: number }): <E, R>(self: HttpClient.HttpClient.With<E, R>) => HttpClient.HttpClient.With<E, R>
  <E, R>(self: HttpClient.HttpClient.With<E, R>, options: { readonly timeoutMs?: number }): HttpClient.HttpClient.With<E, R>
} = dual(2, (self, options) =>
  HttpClient.transformResponse(self, Effect.updateService(TxikiOptions, (current) => ({ ...current, ...options })))
)

const transportError = (request: Parameters<HttpClient.HttpClient["execute"]>[0], description: string, cause?: unknown) =>
...
const fromTxikiWeb = (
  ...
  const response = HttpClientResponse.fromWeb(request, source) as HttpClientResponse.HttpClientResponse & {
...
const encodeMultipartFormData = async (
  ...
  const entries = (formData as FormData & {
...
): Effect.Effect<{ readonly body: BodyInit | null; readonly contentType?: string }, HttpClientError.HttpClientError> => {
  switch (body._tag) {
    case "Empty":
    ...
    case "Raw": {
      ...
      if (ArrayBuffer.isView(body.body)) {
        return Effect.succeed({
          body: toArrayBuffer(new Uint8Array(body.body.buffer, body.body.byteOffset, body.body.byteLength))
        })
      }
      ...
      try: async () =>
        fetch(url, {
          ...
          headers: {
            ...
            ...(encodedBody.contentType === undefined ? {} : { "content-type": encodedBody.contentType })
          },
