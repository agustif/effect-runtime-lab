...
type TjsGlobal = {
  ...
  makeDir(path: string, options?: { readonly mode?: number; readonly recursive?: boolean }): Promise<void>
  ...
  remove(path: string, options?: { readonly maxRetries?: number; readonly retryDelay?: number }): Promise<void>
  link(path: string, newPath: string): Promise<void>
  symlink(path: string, newPath: string, options?: { readonly type?: "file" | "directory" | "junction" }): Promise<void>
  watch(path: string, handler: (filename: string, event: "change" | "rename") => void): { readonly path: string; close(): void }
}
...
const systemError = (method: string, pathOrDescriptor?: string | number) => (cause: unknown) => {
  ...
  const tag: PlatformError.SystemErrorTag = code === "ENOENT"
    ? "NotFound"
    : code === "EBADF"
    ? "BadResource"
...
const joinPath = (left: string, right: string) => (left.endsWith("/") ? `${left}${right}` : `${left}/${right}`)
const tempTemplate = (prefix: string | undefined, suffix = "") => `${prefix ?? "tmp-"}XXXXXX${suffix}`
...
const readDirectoryImpl = async (tjs: TjsGlobal, path: string, recursive: boolean): Promise<Array<string>> => {
...
const copyRecursive = async (tjs: TjsGlobal, fromPath: string, toPath: string, overwrite: boolean): Promise<void> => {
  ...
  if (stat.isDirectory) {
    ...
    try {
      for await (const entry of dir) {
        await copyRecursive(tjs, joinPath(fromPath, entry.name), joinPath(toPath, entry.name), overwrite)
      }
    } finally {
      await dir.close()
    }
  ...
  if (stat.isSymbolicLink) {
    ...
    if (overwrite) {
      try { await tjs.remove(toPath) } catch {}
    }
...
const makeHandle = (handle: TjsFileHandle, append: boolean): FileSystem.File => {
  ...
  return {
    ...
    stat: Effect.tryPromise({ try: () => handle.stat().then(makeInfo), catch: systemError("stat", handle.path) }),
    seek: (offset, from) => Effect.sync(() => {
    ...
    readAlloc: (size) => Effect.tryPromise({
      try: async () => {
    ...
    truncate: (length) => Effect.tryPromise({
      try: async () => {
    ...
    write: (buffer) => Effect.tryPromise({
      try: async () => {
    ...
    writeAll: (buffer) => Effect.tryPromise({
      try: async () => {
        let offset = 0
...
const access: FileSystem.FileSystem["access"] = (path, options) =>
  Effect.flatMap(getTjs("access"), (tjs) => Effect.tryPromise({
    try: async () => {
      await tjs.stat(path)
...
const makeDirectory = (path: string, options?: { readonly recursive?: boolean | undefined; readonly mode?: number | undefined }) =>
  Effect.flatMap(getTjs("makeDirectory"), (tjs) => Effect.tryPromise({
    try: () => tjs.makeDir(path, {
...
const remove = (path: string, options?: { readonly recursive?: boolean | undefined; readonly force?: boolean | undefined }) =>
  Effect.flatMap(getTjs("remove"), (tjs) => Effect.tryPromise({
    try: async () => {
      try {
        await tjs.remove(path)
      } catch (cause) {
        if (options?.force && typeof cause === "object" && cause !== null && "code" in cause && String((cause as { code: unknown }).code) === "ENOENT") {
          return
        }
        throw cause
      }
...
const open: FileSystem.FileSystem["open"] = (path, options) =>
  Effect.acquireRelease(
    Effect.flatMap(getTjs("open"), (tjs) => Effect.tryPromise({ try: () => tjs.open(path, options?.flag ?? "r", options?.mode), catch: systemError("open", path) })),
    (handle) => Effect.ignore(Effect.tryPromise({ try: () => handle.close(), catch: systemError("open", handle.path) }))
...
const writeFile: FileSystem.FileSystem["writeFile"] = (path, data, options) =>
  Effect.acquireUseRelease(
    ...
    (handle) => Effect.ignore(Effect.tryPromise({ try: () => handle.close(), catch: systemError("writeFile", handle.path) }))
  )

const fileSystem = FileSystem.make({
  access,
  chmod: (path, mode) => Effect.flatMap(getTjs("chmod"), (tjs) => Effect.tryPromise({ try: () => tjs.chmod(path, mode), catch: systemError("chmod", path) })),
  chown: (path, uid, gid) => Effect.flatMap(getTjs("chown"), (tjs) => Effect.tryPromise({ try: () => tjs.chown(path, uid, gid), catch: systemError("chown", path) })),
  copy: (fromPath, toPath, options) => Effect.flatMap(getTjs("copy"), (tjs) => Effect.tryPromise({ try: () => copyRecursive(tjs, fromPath, toPath, options?.overwrite ?? false), catch: systemError("copy", fromPath) })),
  copyFile: (fromPath, toPath) => Effect.flatMap(getTjs("copyFile"), (tjs) => Effect.tryPromise({ try: () => tjs.copyFile(fromPath, toPath), catch: systemError("copyFile", fromPath) })),
  link: (fromPath, toPath) => Effect.flatMap(getTjs("link"), (tjs) => Effect.tryPromise({ try: () => tjs.link(fromPath, toPath), catch: systemError("link", fromPath) })),
  makeDirectory,
  makeTempDirectory: (options) => Effect.flatMap(getTjs("makeTempDirectory"), (tjs) => Effect.tryPromise({ try: () => tjs.makeTempDir(joinPath(options?.directory ?? (tjs.tmpDir ?? "."), tempTemplate(options?.prefix))), catch: systemError("makeTempDirectory") })),
  makeTempDirectoryScoped: (options) => Effect.acquireRelease(
    Effect.flatMap(getTjs("makeTempDirectoryScoped"), (tjs) => Effect.tryPromise({ try: () => tjs.makeTempDir(joinPath(options?.directory ?? (tjs.tmpDir ?? "."), tempTemplate(options?.prefix))), catch: systemError("makeTempDirectoryScoped") })),
    (path) => Effect.ignore(remove(path, { recursive: true, force: true }))
  ),
  makeTempFile: (options) => Effect.flatMap(getTjs("makeTempFile"), (tjs) => Effect.tryPromise({
    try: async () => {
      const handle = await tjs.makeTempFile(joinPath(options?.directory ?? (tjs.tmpDir ?? "."), tempTemplate(options?.prefix)))
  ...
  makeTempFileScoped: (options) => Effect.acquireRelease(
    Effect.flatMap(getTjs("makeTempFileScoped"), (tjs) => Effect.tryPromise({
      try: async () => {
        const handle = await tjs.makeTempFile(joinPath(options?.directory ?? (tjs.tmpDir ?? "."), tempTemplate(options?.prefix)))
        const path = await renameWithSuffix(tjs, handle.path, options?.suffix)
        await handle.close()
  ...
  open,
  readDirectory: (path, options) => Effect.flatMap(getTjs("readDirectory"), (tjs) => Effect.tryPromise({ try: () => readDirectoryImpl(tjs, path, options?.recursive ?? false), catch: systemError("readDirectory", path) })),
  readFile: (path) => Effect.flatMap(getTjs("readFile"), (tjs) => Effect.tryPromise({ try: () => tjs.readFile(path), catch: systemError("readFile", path) })),
  readLink: (path) => Effect.flatMap(getTjs("readLink"), (tjs) => Effect.tryPromise({ try: () => tjs.readLink(path), catch: systemError("readLink", path) })),
  realPath: (path) => Effect.flatMap(getTjs("realPath"), (tjs) => Effect.tryPromise({ try: () => tjs.realPath(path), catch: systemError("realPath", path) })),
  remove,
  rename: (oldPath, newPath) => Effect.flatMap(getTjs("rename"), (tjs) => Effect.tryPromise({ try: () => tjs.rename(oldPath, newPath), catch: systemError("rename", oldPath) })),
  stat: (path) => Effect.flatMap(getTjs("stat"), (tjs) => Effect.tryPromise({ try: () => tjs.stat(path).then(makeInfo), catch: systemError("stat", path) })),
  symlink: (fromPath, toPath) => Effect.flatMap(getTjs("symlink"), (tjs) => Effect.tryPromise({ try: () => tjs.symlink(fromPath, toPath), catch: systemError("symlink", fromPath) })),
  truncate: (path, length) =>
    Effect.acquireUseRelease(
      Effect.flatMap(getTjs("truncate"), (tjs) => Effect.tryPromise({ try: () => tjs.open(path, "r+"), catch: systemError("truncate", path) })),
      (handle) => Effect.tryPromise({ try: () => handle.truncate(length === undefined ? undefined : Number(length)), catch: systemError("truncate", path) }),
      (handle) => Effect.ignore(Effect.tryPromise({ try: () => handle.close(), catch: systemError("truncate", path) }))
    ),
  utimes: (path, atime, mtime) => Effect.flatMap(getTjs("utimes"), (tjs) => Effect.tryPromise({ try: () => tjs.utime(path, atime instanceof Date ? atime : new Date(atime), mtime instanceof Date ? mtime : new Date(mtime)), catch: systemError("utimes", path) })),
  watch: (path) =>
