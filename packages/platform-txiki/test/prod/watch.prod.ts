...
const main = Effect.gen(function* () {
  ...
  const dir = "./tmp-txiki-watch";
  const file = `${dir}/watched.txt`;

  yield* fs.makeDirectory(dir, { recursive: true });

  const fiber = yield* fs.watch(dir).pipe(Stream.take(4), Stream.runCollect, Effect.forkChild);
  ...
  yield* Effect.promise(() =>
    ConformanceWatch.verifyWatchRenameLayer(TxikiFileSystem.layer, dir, file, renamed),
  );
  ...
  yield* Effect.promise(() =>
    ConformanceWatch.verifyWatchChurnLayer(TxikiFileSystem.layer, nestedDir, [
