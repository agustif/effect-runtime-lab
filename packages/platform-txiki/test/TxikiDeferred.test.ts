...
describe("Txiki deferred parity surfaces", () => {
  ...
  it("captures executable, dependency, startup, and memory proofs from this repo", () => {
    ...
    expect(bench.startup.some((entry) => entry.name === "txiki-runtime" && entry.maxResidentSetKb !== null)).toBe(true)
    expect(
      bench.warmStart.some(
        (entry) => entry.name === "txiki-runtime" && entry.rssSamplesKb.some((value) => value !== null)
      )
    ).toBe(true)
  })
})
